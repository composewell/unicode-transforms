...
decompositionOf _c (DC ds) = ds

filterProps :: (CharProps -> Bool) -> PropertiesDB -> IM.IntMap [Char]
filterProps filt = IM.fromList
                . map (\(c,prop) -> (ord c, decompositionOf c (_decomposition prop)))
                . filter (\(_,prop) -> filt prop
                                      && _decomposition prop /= DCSelf)

decompositions :: PropertiesDB -> IM.IntMap [Char]
decompositions = filterProps ((== Just DTCanonical) . _decompositionType)

genDecompositions :: FilePath -> PropertiesDB -> IO ()
genDecompositions outdir props = do
  genFile outdir "DecompD" "IntMap [Char]" (show $ decompositions props)
