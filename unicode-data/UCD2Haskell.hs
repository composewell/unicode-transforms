...
genDecompositions dir props = do
    writeFile (dir <> "/" <> file <> ".hs") $
        unlines
            ...
            , identifier <> " c | isDecomposable c == False = [c]"
            , ""
            , concat $ map genDecomposeDef decompList
            , identifier <> " c = [c]"
            , ""
            , "isDecomposable :: Char -> Bool"
            , "isDecomposable c | (ord c) < "
              <> show (minimum decompOrds) <> " || (ord c) > "
              <> show (maximum decompOrds) <> " = False"
            , "isDecomposable c = lookupBit decomposeArray (ord c)"
            , ""
            , "decomposeList :: [(Int, Bool)]"
            , "decomposeList = " ++ (show $ map (,True) decompOrds)
            , ""
            , "decomposeArray :: BitArray"
            , "decomposeArray = bitArray "
              ++ (show (minimum decompOrds, maximum decompOrds))
              ++ " decomposeList"
            ]
...
-}
    where
        ...
        decompList = decompositions props
        decompOrds = map (ord . fst) decompList
        genDecomposeDef (c, d) =
            identifier <> " " <> show c <> " = " <> show d <> "\n"
