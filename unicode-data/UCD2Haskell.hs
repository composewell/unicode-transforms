...
import           Data.Unicode.Properties.DecomposeHangul (hangulFirst,
                                                          hangulLast)
...
genDecomposable :: PropertiesDB -> String -> String
genDecomposable props file = unlines
            [ "-- autogenerated from Unicode data"
            , "module Data.Unicode.Properties." <> file
            , "(isDecomposable)"
            , "where"
...
genDecompositions props file = unlines
            ...
            , concat $ map (genDecomposeDef "decomposeCharLow") lowDecomps
            , ""
            , "decomposeCharHigh :: Char -> [Char]"
            , "{-# NOINLINE decomposeCharHigh #-}"
            , concat $ map (genDecomposeDef "decomposeCharHigh") highDecomps
            ]

{-
    putStrLn $ unlines . map showRange
             $ splitSeq splitter
             $ map (fst) (decompositions props)
-}
    where
        -- splitter a b = ord b <= (ord a + 1024)
        -- showRange xs = show (head xs) ++ ".." ++ show (last xs) ++ " : " ++ show (length xs)

        decomps     = decompositions props
        lowDecomps  = filter ((< hangulFirst) . ord . fst) decomps
        highDecomps = filter ((> hangulLast) . ord . fst) decomps
...
processFile src outdir = do
    ...
    print $ length props
    emitFile "Decomposable"   $ genDecomposable   props
